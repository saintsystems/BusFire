...
1. **Producer — `IBus`/`Bus`:** `Send`/`Publish` run handlers **inline** via `IBusInternal` unless the message implements `IShouldQueue`, in which case they enqueue a Hangfire job targeting `HangfireBridge`. `IQueueable : IShouldQueue` adds self-declared `Queue`/`Delay` (read-only getters); queue precedence is per-call arg › `IQueueable.Queue` › `"default"`, and a non-null `IQueueable.Delay` schedules instead of enqueues. `Defer` always enqueues (`Schedule`) — a delayed message can't run inline "now". **Recurring (cron) dispatch** is a fourth trigger: `IBusFireScheduler` (`Infrastructure/BusFireScheduler.cs`) registers Hangfire recurring jobs whose body is the same `HangfireBridge.Send`/`Publish`, so scheduled work reuses the identical pipeline; the scheduler depends on the bus/bridge, never the reverse.
...
- **Git workflow:** branch off `main` → **pull request** → squash-merge. Commit and **PR titles follow [Conventional Commits](https://www.conventionalcommits.org/)** (`feat:`/`fix:`/`docs:`/`refactor:`/`test:`/`chore:`/`ci:`/`build:`/`perf:`); the title drives auto-labeling + categorized release notes (`.github/release.yml`). Releases are tag-driven (`git tag vX.Y.Z && git push origin vX.Y.Z` → publish + GitHub Release). See [`CONTRIBUTING.md`](CONTRIBUTING.md).
